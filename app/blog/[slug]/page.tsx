...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const [artikel] = await db
    .select({
      ...
      metaTitel: artikelen.metaTitel,
      metaBeschrijving: artikelen.metaBeschrijving,
  ...
  return {
    ...
    openGraph: {
      ...
      siteName: "Brickme",
      type: "article",
      publishedTime: artikel.gepubliceerdOp?.toISOString(),
      images: artikel.ogAfbeelding ? [{ url: artikel.ogAfbeelding, width: 1200, height: 630 }] : undefined,
    },
    twitter: {
      card: "summary_large_image",
      title: titel,
      description: beschrijving,
      images: artikel.ogAfbeelding ? [artikel.ogAfbeelding] : undefined,
    },
  };
}
    ...
    <>
      ...
      <main style={{ minHeight: "100vh", background: "var(--color-surface)" }}>
        ...
        <header style={{ maxWidth: 760, margin: "0 auto", padding: "40px 40px 32px" }}>
          ...
          <h1 style={{ fontSize: 42, fontWeight: 400, margin: "0 0 20px", lineHeight: 1.15 }}>
            {artikel.titel}
          </h1>
          {artikel.excerpt && (
            <p style={{ fontSize: 18, color: "var(--color-text-muted)", margin: "0 0 24px", lineHeight: 1.6 }}>
              {artikel.excerpt}
            </p>
          )}
        ...
        {artikel.tags && artikel.tags.length > 0 && (
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 40px 60px", borderTop: "1px solid var(--color-outline)", paddingTop: 32 }}>
        ...
        {/* Terug naar blog */}
        <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 40px 80px", borderTop: "1px solid var(--color-outline)", paddingTop: 40 }}>
          <Link href="/blog" style={{ fontSize: 15, color: "var(--color-primary)", textDecoration: "none", fontWeight: 500 }}>
            ← Alle artikelen
          </Link>
        </div>
      </main>
